<template>
	<div
		...
		:id="'cloud_window' + id"
	>
		<div class="header" :style="{ background: !this.config.frame && this.config.hideTitle ? 'none' : this.config.background }" @dblclick="restore">
			<div class="left" v-if="this.config.frame">
				<img v-if="config.img" :src="config.img" draggable="false" alt="" />
				<span v-if="!this.config.hideTitle" :style="{ color: this.config.color }">{{ config.title }}</span>
			</div>
		...
		<div class="container" :class="windowClass"></div>
	...
	computed: {
		...
		windowClass: function () {
			return {
				...
				frameMaxHeight: !this.config.frame && this.state === 'max',
			};
		},
	},
	methods: {
		init(config) {
			...
			this.minTop = this.$projectInfo.projectName === 'index' ? 40 : 0;
		...
		restore() {
			...
			if (this.state === 'restore') {
				...
				this.win.style.top = this.minTop + 'px';
		...
		active(callback) {
			...
			callback && callback(this.component, this);
			return false;
		},
	},
};
